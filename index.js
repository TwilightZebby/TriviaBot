    ...
    if ( !prefixRegex.test(message.content) ) {
        ...
        if ( command.limitation ) {
            switch(command.limitation) {
                ...
                case "twilightzebby":
                    // Only myself, as the Bot's developer, can use this command
                ...
                case "owner":
                    // Only myself and Dr1fterX can use this command
                    if ( message.author.id !== "156482326887530498" || message.author.id !== "136391162876395520" )
                    {
                        return await message.channel.send(`**${message.member.displayName}** sorry, but this command is limited to the Server Owner`);
                    }
                    break;
